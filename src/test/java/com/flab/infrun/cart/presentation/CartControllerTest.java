...
final class CartControllerTest {
    ...
    void addCartItem_success() throws Exception {
        ...
        when(cartFacade.addCartItem(any()))
        ...
        result.andExpect(jsonPath("$.data.totalPrice").value(10000));
        result.andExpect(jsonPath("$.data.lectureIds").isNotEmpty());
        result.andExpect(jsonPath("$.data.lectureIds").value(containsInAnyOrder(1, 2, 3)));
    }
    ...
    void deleteCartItem_success() throws Exception {
        ...
        when(cartFacade.deleteCartItem(any()))
        ...
        result.andExpect(status().isOk());
        result.andExpect(jsonPath("$.data.totalPrice").value(5000));
        result.andExpect(jsonPath("$.data.lectureIds").isNotEmpty());
        result.andExpect(jsonPath("$.data.lectureIds").value(containsInAnyOrder(2, 3)));
    }
